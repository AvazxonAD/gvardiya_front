...
interface Props {
  data: SoldierTasksResponse | null;
}
...
export default function SoldierTasksChart({ data }: Props) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const chartRef = useRef<Chart | null>(null);
  const sortedRef = useRef<SoldierTaskRow[]>([]);
  ...
  useEffect(() => {
    ...
    const labels = sorted.map((r) => {
      const percent = d.total_summa > 0 ? Math.round((r.total_summa / d.total_summa) * 100) : 0;
      return `${r.batalon_name} — ${formatNum(r.total_summa)} (${percent}%)`;
    });
    ...
    const insideLabelsPlugin = {
      ...
      afterDraw(chart: Chart) {
        ...
        meta.data.forEach((arc: any, i: number) => {
          ...
          c.save();
          c.fillStyle = "#ffffff";
          c.font = percent < 5 ? "bold 9px Inter, sans-serif" : "bold 12px Inter, sans-serif";
          c.textAlign = "center";
          c.textBaseline = "middle";
          c.fillText(`${Math.round(percent)}%`, x, y);
          c.restore();
        });
      },
    };
